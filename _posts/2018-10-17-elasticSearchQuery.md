...
6、因为单节点（硬件限制）不可能存储太大的数据量，es提供了将index（一组document的集合）划分为分片的功能。分片数据只是整个index数据的一部分，每个分片在自身内部都是一个有着完整功能并且独立的“索引”，可以被托管到集群中的任意一个节点中,而一个分片(shard)又包含多个segment，而每一个segment是一个倒排索引，每针对一个shard的查询操作都将被归于对该shard中的segment的查询。<br>
...
9、NRT近实时。从我们对一个文档进行创建索引开始到这个文档变得可以被搜索之间可能有一个潜在的传输延迟时间（默认是1秒,可以进行强制刷新以让数据成为可检索状态）。<br>

## 重点知识
#### 倒排索引
&nbsp;&#8195;一个文档首先会经过分词器的分词，将其拆分为一个词汇集合。词汇集合和文档会被ES引擎写入到倒排索引的数据结构里面。倒排索引即是
词汇和文档之间的映射关系。举例：中国江西省广丰区(文档1)。 经过分词器得到分词为：中国、江西省、广丰区。<br>
...
&nbsp;&#8195;实际上的倒排索引记录的信息远不止这么简单，其还会记录譬如词汇在每个文档中出现的次数、每个文档包含多少单词等信息。这些信息会用作给搜索结果打分。<br>
倒排索引是不可修改的，一旦建立，里面的数据就不会再次进行变更。<br>
&nbsp;&#8195;其带来的好处：
...
既然倒排索引是不可修改的，那么如何添加新的数据、删除数据以及更新数据？很明显整个ES集群不可能就是一张大的倒排索引表。lucene（ES全文检索底层实现）会将一个大的倒排索引拆分为多个小的segment。每个segment本质上就是一个倒排索引。在lucene中还会维护一个文件的提交点（commit point），记录当前所有可以用的segment，当我们在这个commit point上进行搜索时，就相当于在它下面的segment进行搜索，每个segment返回自己的搜索结果，然后进行汇总返回给用户。从这里可以得知，segment其实是一直在增加的，无论是删除还是更新。那么如何实现文档的更新和删除，在每个commit point都会维护一个.del文件，文件内记录了在某个segment内某个文档已经被删除。在segment中，被删除的文档依旧是能够被搜索到的，不过在返回搜索结果前，会根据.del把那些已经被删除的文档从索索结果中过滤掉。<br>
&nbsp;&#8195;以上的个人理解：因为倒排索引是不支持修改的，那么整个集群中segment的数据量是一直在增加的，commit point很像一个存档点的概念。每次我们只读取commit point之前的所有segment进行相关的文档检索，那么很明显有些已经被删除的文档会被检索到，又经.del文件中的针对某个segment内某个文档已经被删除的记录过滤，我们最终得到了准确的检索文档id。<br>

#### 读写模型
![写模型](/img/writeModel.png)
主分片处理流程：<br>
&nbsp;&#8195;1、验证请求是否符合Elasticsearch的接口规范，如果不符合，直接拒绝。<br>
&nbsp;&#8195;2、在主分片上执行操作（例如索引、更新或删除一个文档）。如果执行过程中出错，直接返回错误。<br>
&nbsp;&#8195;3、将操作转发到当前同步副本集的每个副本。如果有多个副本，则并行执行。（in-sync当前可用、激活的副本）。<br>
&nbsp;&#8195;4、一旦所有的副本成功地执行了操作并对主服务器进行了响应，主服务器就会承认对客户机的请求的成功完成。<br>

那么写的时候发生问题了？<br>
（1）在主服务器本身发生故障的情况下，托管主服务器的节点将向主服务器发送有关它的消息。<br>
（2）索引操作将等待（默认情况下最多1分钟），以便主服务器将其中一个副本提升为新的主数据库。<br>
（3）该操作将被转发到新的主要处理。请注意，主服务器还会监控节点的运行状况，<br>
（4）并可能决定主动降级主服务器。<br>
（5）新的主服务器发出删除所有错误分片命令。<br>
（6）所有错误分片删除ACK传回主服务器。<br>
（7）请求将被路由到新的主服务器上。<br>
...
&nbsp;&#8195;业务大致：O-->A-->B。数据源是O,每处理一个O的元数据会将相关的数据录入到ES中A的index,同时会发送一个消息给B系统。B从A录入到ES的index检索数据，经过一系列操作之后将产生的数据录入到ES中B的index。<br>
&nbsp;&#8195;在B录入数据的过程中发现同样一个dsl不同时间查询的数据量是不一样的。原因是：搜索系统的Index一般都是NRT（Near Real Time），近实时的，比如Elasticsearch中，Index的实时性是由refresh控制的，默认是1s，最快可到100ms，那么也就意味着Index doc成功后，需要等待一秒钟后才可以被搜索到。<br>
**什么是refresh？**<br>
